      ...
      <Section background="page-bg" padding="lg">
        <Container>
          <section className="py-12">
            <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-screen-xl">
              <h2 className="text-3xl font-bold text-heading mb-8 text-center">Unsere Führerscheinklassen</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {klassenFeatures.map((feature, index) => (
                  <KartenSpotlight
                    key={index}
                    className="hover:shadow-lg hover:border-primary-500 transition-all duration-300 group flex flex-col h-full p-6"
                    spotlightColor={(feature.spotlightColor as 'primary' | 'secondary' | 'brand-500' | 'status-success' | 'status-warning' | 'status-error' | 'status-info') || 'primary'}
                  >
